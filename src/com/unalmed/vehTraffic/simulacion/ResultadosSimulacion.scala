...
//  val promedioOrigen= {val listaCantidad = simulacion.listaVehiculos.groupBy(_.viaje.origen).mapValues(_.size).toArray.map(_._2)
...
//  val promedioDestino = {val listaCantidad = simulacion.listaVehiculos.groupBy(_.viaje.destino).mapValues(_.size).toArray.map(_._2)
//                       listaCantidad.reduce(_+_) / listaCantidad.length}
//  result.resultadosSimulacion.mallaVial.vehiculosEnInterseccion.promedioDestino=promedioDestino
//  val sinOrigen = simulacion.listaIntersecciones.diff(simulacion.listaVehiculos.groupBy(_.viaje.origen).toArray.map(_._1)).length
//  result.resultadosSimulacion.mallaVial.vehiculosEnInterseccion.sinOrigen=sinOrigen
//  val sinDestino = simulacion.listaIntersecciones.diff(simulacion.listaVehiculos.groupBy(_.viaje.destino).toArray.map(_._1)).length
...
//  
//  //Distancia vehículos
//  val distanciaMinima = simulacion.listaVehiculos.map(_.viaje.camino.get.edges.map(_.label.asInstanceOf[Via].longitud).toList.reduce(_+_)).min
//  result.resultadosSimulacion.distancias.minima=distanciaMinima.toInt
//  val distanciaMaxima = simulacion.listaVehiculos.map(_.viaje.camino.get.edges.map(_.label.asInstanceOf[Via].longitud).toList.reduce(_+_)).max
//  result.resultadosSimulacion.distancias.maxima=distanciaMaxima.toInt
//  val distanciaPromedio = simulacion.listaVehiculos.map(_.viaje.camino.get.edges.map(_.label.asInstanceOf[Via].longitud).toList.reduce(_+_)).reduce(_+_)/totalVehiculos
